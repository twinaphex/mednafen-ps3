...
namespace Nes
{
	namespace Core
	{
		...
		dword Cpu::GetTime(Cycle clock) const
		{
			return
			(
				model == CPU_RP2A03 ? clock * queword( CPU_RP2A03_CC * CLK_NTSC_DIV ) / CLK_NTSC :
				model == CPU_RP2A07 ? clock * queword( CPU_RP2A07_CC * CLK_PAL_DIV  ) / CLK_PAL  :
                                      clock * queword( CPU_DENDY_CC  * CLK_PAL_DIV  ) / CLK_PAL
			);
		}
