...
#ifndef MDFNPS3 //ROBO: Load ROM from memory
int CPULoadRom(const char *szFile)
#else
int CPULoadRom(const unsigned char *szFile, uint32_t length)
#endif
{
  romSize = 0x2000000;
  if(rom != NULL) {
  ...
  if(szFile!=NULL)
  {
#ifndef MDFNPS3 //ROBO: Load File from memory
	  if(!utilLoad(szFile,
						  utilIsGBAImage,
						  whereToLoad,
						  romSize)) {
...
#else
      memcpy(whereToLoad, szFile, length);
      romSize = length;
#endif
  }
