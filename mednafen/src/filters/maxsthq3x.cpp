...
static void filter(uint32_t *pOut, const unsigned dstPitch,
...
void MaxSt_Hq3x::init(uint32_t aWidth, uint32_t aHeight) {
	delete []buffer;
	delete []obuffer;
	...
	obuffer = new uint32_t[(width * 3) * (height * 3)];
}
...
const FilterInfo& MaxSt_Hq3x::info() {
	static const FilterInfo fInfo = { "MaxSt's Hq3x", 3, 3 };
	return fInfo;
}

uint32_t* MaxSt_Hq3x::inBuffer() {
	return buffer;
}
...
void MaxSt_Hq3x::filter() {
	::filter(obuffer, width * 3, buffer, width, height);
}

uint32_t* MaxSt_Hq3x::outBuffer() {
	return obuffer;
}

uint32_t MaxSt_Hq3x::outPitch() {
	return width * 3;
}

uint32_t MaxSt_Hq3x::getWidth() {
	return width;
}

uint32_t MaxSt_Hq3x::getHeight() {
	return height;
}
