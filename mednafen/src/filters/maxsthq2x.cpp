...
void MaxSt_Hq2x::init(uint32_t aWidth, uint32_t aHeight) {
	delete []buffer;
	delete []obuffer;
	...
	buffer = new uint32_t[aWidth * aHeight];
...
const FilterInfo& MaxSt_Hq2x::info() {
	static const FilterInfo fInfo = { "MaxSt's Hq2x", 2, 2 };
	return fInfo;
}
...
void MaxSt_Hq2x::filter() {
	::filter(obuffer, width * 2, buffer, width, height);
}


uint32_t* MaxSt_Hq2x::outBuffer() {
	return obuffer;
}

uint32_t MaxSt_Hq2x::outPitch() {
	return width * 2;
}

uint32_t MaxSt_Hq2x::getWidth() {
	return width;
}

uint32_t MaxSt_Hq2x::getHeight() {
	return height;
}
