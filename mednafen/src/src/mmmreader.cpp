...
// FIXME TODO: Clean up memory allocation and free'ing on error handling
MMM_Stream_Frame *MMM_Reader::ReadFrame(void)
{
 MM_Frame_Header frame_header;

 if(fread(&frame_header, 1, sizeof(MM_Frame_Header), fp) == sizeof(MM_Frame_Header))
 {
  if(!memcmp(frame_header.magic, "FRAME\0\0\0", 8))
  {
   MMM_Stream_Frame *frame = (MMM_Stream_Frame *)MDFN_calloc(1, sizeof(MMM_Stream_Frame), "MMM_Stream_Frame");
   ...
   width = MDFN_delsb(&frame_header.width);
   height = MDFN_delsb(&frame_header.height);
   c_len = MDFN_delsb(&frame_header.compressed_size);
   uc_len = MDFN_delsb(&frame_header.uncompressed_size);
   audio_frames = MDFN_delsb(&frame_header.audio_frame_count);
   ...
   if(width > max_width)
   {
    MDFN_PrintError("Video frame width field exceeds max_width in main header.");
    FreeFrame(frame);
    return(NULL);
   }
   ...
   {
    MDFN_PrintError("Video frame height field exceeds max_height in the main header.");
    FreeFrame(frame);
    return(NULL);
   }
   ...
   if(!width)
   {
    ...
    if(fread(frame->line_widths, sizeof(uint16), height, fp) != height)
    {
     ErrnoHolder ene(errno);

     MDFN_PrintError("Error reading line widths: %s", ene.StrError());
     FreeFrame(frame);
     return(NULL);
    }
    Endian_A16_LE_to_NE(frame->line_widths, height);

    for(int y = 0; y < height; y++)
    {
     if(frame->line_widths[y] > max_width)
     {
      MDFN_PrintError("Line width for line %d exceeds max width in main header.", y);
      FreeFrame(frame);
      return(NULL);
     }
     else if(!frame->line_widths[y])
     {
      MDFN_PrintError("Line width for line %d is zero!", y);
      FreeFrame(frame);
      return(NULL);
     }
    }
   ...
   if(fread(compressed_buffer, 1, c_len, fp) != c_len)
   {
    ErrnoHolder ene(errno);

    MDFN_PrintError("Error reading compressed video data: %s", ene.StrError());
    return(NULL);
   }
   ...
   if(fread(frame->audio, sizeof(int16) * soundchan, audio_frames, fp) != audio_frames)
   {
    ErrnoHolder ene(errno);

    MDFN_PrintError("Error reading audio data: %s", ene.StrError());

    FreeFrame(frame);
    return(NULL);
   }
   Endian_A16_LE_to_NE(frame->audio, soundchan * audio_frames);
