...
class VCE : public HuC6280_Support
{
	...
	void RunFrame(MDFN_Surface *surface, MDFN_Rect *DisplayRect, MDFN_Rect *LineWidths, int skip);
	...
	bool NeedSLReset;
        ...
        uint8 dot_clock;	// CR->Dot Clock(5, 7, or 10 MHz = 0, 1, 2)
	uint8 dot_clock_lc;	// CR->Dot clock line cache.
	int32 dot_clock_ratio;	// CR->Dot clocck ratio cache

	int32 dot_clock_fudge_counter;
