...
static const MDFNCS *FindSetting(const char *name, bool dont_freak_out_on_fail = false);
...
static INLINE void MergeSettingSub(const MDFNSetting *setting)
{
  ...
  if(FindSetting(setting->name, true) != NULL)
  {
...
//ROBO: Don't clear them, we really need them
...
static const MDFNCS *FindSetting(const char *name, bool dont_freak_out_on_fail)
{
