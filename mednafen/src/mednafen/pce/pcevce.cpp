...
// TODO: Fix horizontal offset in 7MHz dotclock mode when this is defined to 1
...
bool VCE::WS_Hook(int32 vdc_cycles)
{
 ...
 //return(0);
#if 0
 int32 to_steal = (vdc_cycles * dot_clock_ratio - clock_divider);
 ...
 if((to_steal + ws_counter) > 455 * 8 * 3)
 {
  to_steal = 455 * 8 * 3- ws_counter;
 ...
 if(to_steal > 0)
 {
  HuCPU->StealMasterCycles(to_steal);
  ws_counter += to_steal;
 }
 ...
 int32 to_steal = ((vdc_cycles * dot_clock_ratio - clock_divider) + 2) / 3;
...
#if 0
 if((to_steal + ws_counter) > 455 * 8)
 {
  to_steal = 455 * 8 - ws_counter;
...
void VCE::RunFrame(MDFN_Surface *surface, MDFN_Rect *DisplayRect, MDFN_Rect *LineWidths, int skip)
{
  ...
  case 0: 
	  {
	   ...
	   if(old_dot_clock != dot_clock)
	   {
	    if(dot_clock_fudge_counter < 512)	// See comment below.
	    {
	     clock_divider = 0;
	     dot_clock_fudge_counter++;
	    }

	    //printf("Dot clock change on VCE line %d(VB=%d, HB=%d).  From: %d, To: %d\n", scanline, vblank, hblank, old_dot_clock, dot_clock);

	    if(dot_clock == 2)        // Switching to 10MHz dot clock
	    {
	     if(dot_clock_fudge_counter < 512)	// Limit the maximum timing irregularity to about 1/2 of a scanline's time, due to limitations of Mednafen.
	     {					// Note that "fudging" the counters like this is almost certainly not correct, however, something with
						// similar effects should occur with a real VCE.  And it's needed to fix Asuka 120%.
	      dot_clock_fudge_counter += 63;

	      hblank_counter += 63;
	      vblank_counter += 63;
	     }
	    }
	    else if(old_dot_clock == 2)       // Switch from 10MHz dot clock to something else.
	    {
...
bool VCE::ToggleLayer(int which)
{
 if((which >> 1) >= chip_count)
  return(FALSE);

 return(vdc[which >> 1]->ToggleLayer(which & 1));
}
