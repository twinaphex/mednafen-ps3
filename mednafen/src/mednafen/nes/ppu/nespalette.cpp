...
static bool LoadCPalette(void)
{
  ...
  if(cpalette_fn != "")
  {
   if(!(fp = fopen(cpalette_fn.c_str(), "rb")))
   {
    ...
    MDFN_PrintError(_("Error opening file \"%s\" for \"%s\": %s"), cpalette_fn.c_str(), "nes.cpalette", ene.StrError());
    return(false);
   }
   ...
   if(fread(ptmp, 1, 192, fp) != 192)
   {
    ...
    MDFN_PrintError(_("Error reading file \"%s\" for \"%s\": %s"), cpalette_fn.c_str(), "nes.cpalette",
