...
{
  { "nethost", MDFNSF_NOFLAGS, gettext_noop("Network play server hostname."), NULL, MDFNST_STRING, "fobby.net" },
  { "netport", MDFNSF_NOFLAGS, gettext_noop("Port to connect to on the server."), NULL, MDFNST_UINT, "4046", "1", "65535" },
  { "netpassword", MDFNSF_NOFLAGS, gettext_noop("Password to connect to the netplay server."), NULL, MDFNST_STRING, "" },
  { "netlocalplayers", MDFNSF_NOFLAGS, gettext_noop("Number of local players for network play."), NULL, MDFNST_UINT, "1", "1", "8" },
  { "netnick", MDFNSF_NOFLAGS, gettext_noop("Nickname to use for network play chat."), NULL, MDFNST_STRING, "" },
  { "netgamekey", MDFNSF_NOFLAGS, gettext_noop("Key to hash with the MD5 hash of the game."), NULL, MDFNST_STRING, "" },
  { "netmerge", MDFNSF_NOFLAGS, gettext_noop("Merge input to this player # on the server."), NULL, MDFNST_UINT, "0" },
  { "netsmallfont", MDFNSF_NOFLAGS, gettext_noop("Use small(tiny!) font for netplay chat console."), NULL, MDFNST_BOOL, "0" },

  { "fs", MDFNSF_NOFLAGS, gettext_noop("Enable fullscreen mode."), NULL, MDFNST_BOOL, "0", },
  { "vdriver", MDFNSF_NOFLAGS, gettext_noop("Select video driver, \"opengl\" or \"sdl\"."), NULL, MDFNST_ENUM, "opengl", NULL, NULL, NULL,NULL, VDriver_List },
  { "glvsync", MDFNSF_NOFLAGS, gettext_noop("Attempt to synchronize OpenGL page flips to vertical retrace period."), 
  ...
  { "frameskip", MDFNSF_NOFLAGS, gettext_noop("Enables/Disables frameskip during emulation rendering(disable for rendering code performance testing)."), NULL, MDFNST_BOOL, "1" },
  //{ "frameskip.method", gettext_noop("Selects frame skipping method."), NULL, MDFNST_STRING, "auto" },
  //{ "force_vthread_synch", gettext_noop("If enabled, will force synchronizing with video thread blits.  
	//				   Enabling this will likely cause sound and timing glitches and performance problems, unless
	//				   your monitor is operating at a 120Hz+ refresh rate."), NULL, MDFNST_BOOL, "1" },
  ...
  { "autofirefreq", MDFNSF_NOFLAGS, gettext_noop("Auto-fire frequency."), NULL, MDFNST_UINT, "3", "0", "1000" },
  { "analogthreshold", MDFNSF_NOFLAGS, gettext_noop("Threshold for detecting a \"button\" press on analog axis, in percent."), NULL, MDFNST_FLOAT, "75", "0", "100" },
  { "ckdelay", MDFNSF_NOFLAGS, gettext_noop("The length of time, in milliseconds, that a button/key corresponding to a \"dangerous\" command like power, reset, exit, etc. must be pressed before the command is executed."), NULL, MDFNST_UINT, "0", "0", "99999" },
  { "nothrottle", MDFNSF_NOFLAGS, gettext_noop("Disable speed throttling when sound is disabled."), NULL, MDFNST_BOOL, "0"},
  { "autosave", MDFNSF_NOFLAGS, gettext_noop("Automatically save and load save states when a game is closed or loaded, respectively."), NULL, MDFNST_BOOL, "0"},
  { "sounddriver", MDFNSF_NOFLAGS, gettext_noop("Select sound driver."), NULL, MDFNST_STRING, "default", NULL, NULL },
  { "sounddevice", MDFNSF_NOFLAGS, gettext_noop("Select sound output device."), NULL, MDFNST_STRING, "default", NULL, NULL },
  { "soundvol", MDFNSF_NOFLAGS, gettext_noop("Sound volume level, in percent."), NULL, MDFNST_UINT, "100", "0", "150" },
  { "sound", MDFNSF_NOFLAGS, gettext_noop("Enable sound output."), NULL, MDFNST_BOOL, "1" },
  { "sound.period_time", MDFNSF_NOFLAGS, gettext_noop("If non-zero, specifies the desired period size in microseconds.  Currently only affects OSS and ALSA output."), NULL, MDFNST_UINT,  "0", "0", "100000" },
  { "soundbufsize", MDFNSF_NOFLAGS, gettext_noop("Specifies the desired size of the sound buffer, in milliseconds."), NULL, MDFNST_UINT, 
   #ifdef WIN32
   "52"
   #else
   "32"
   #endif
   ,"1", "1000" },
  { "soundrate", MDFNSF_NOFLAGS, gettext_noop("Specifies the sound playback rate, in frames per second(\"Hz\")."), NULL, MDFNST_UINT, "48000", "22050", "48000"},
  { "helpenabled", MDFNSF_NOFLAGS, gettext_noop("Enable the help screen."), NULL, MDFNST_BOOL, "1" },
...
{
 const char *subs[7] = { "mcs", "mcm", "snaps", "gameinfo", "sav", "cheats", "firmware" };
 char *tdir;
...
static int DoArgs(int argc, char *argv[], char **filename)
{
        ...
        ARGPSTRUCT MDFNArgs[] = 
	{
	 ...
	 { "mmrecord", _("Record video and audio output to the specified filename."), 0, &mmrecfn, SUBSTYPE_STRING_ALLOC }, // TODOC: Video recording done without filtering applied.
...
int64 Time64(void)
{
 ...
 if(clock_gettime(CLOCK_MONOTONIC, &tp) == -1)
 {
  ...
  return((int64)tp.tv_sec * 1e6 + tp.tv_nsec / 1e3);
	...
	//fskip = 1;
	 ...
	 LockGameMutex(1);
	 {
	  ...
	  espec.SoundBuf = GetEmuModSoundBuffer(&espec.SoundBufMaxSize);
 	  espec.SoundVolume = (double)MDFN_GetSettingUI("soundvol") / 100;
	  ...
	  sound = espec.SoundBuf;
	  ssize = espec.SoundBufSize;
	 }
	 LockGameMutex(0);
	 FPS_IncVirtual();
	 if(!fskip)
...
{
        desiredfps = ((double)CurGame->fps * multiplier) / 256;
	tfreq=10000000;
...
MDFN_Thread *MDFND_CreateThread(int (*fn)(void *), void *data)
{
 ...
 if(!(thread->data = SDL_CreateThread(fn, data)))
 {
  free(thread);
  return(NULL);
 }
...
{
 SDL_KillThread((SDL_Thread *)(thread->data));
}
...
{
 return SDL_mutexP((SDL_mutex *)(mutex->data));
}

int MDFND_UnlockMutex(MDFN_Mutex *mutex)
{
 return SDL_mutexV((SDL_mutex *)(mutex->data));
}
