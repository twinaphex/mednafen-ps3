...
/* Elements of these structures that are multi-byte variables(uint16, uint32, uint64)
...
typedef struct
{
 /* 0x00 */ uint8 magic[8];		// "FRAME\0\0\0"
 /* 0x08 */ uint16 width;
 /* 0x0A */ uint16 height;
 /* 0x0C */ uint32 flags;
 /* 0x10 */ uint32 compressed_size;
 /* 0x14 */ uint32 uncompressed_size;
 /* 0x18 */ uint32 audio_frame_count;
 /* 0x1C */ uint8 padding[4];
} __attribute__((__packed__)) MM_Frame_Header;

//#define MM_FHFLAG_DELTAFRAME	0x00000001
