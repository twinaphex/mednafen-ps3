...
void VDC::RunSATDMA(int32 cycles, bool force_completion)
{
 ...
 if(sat_dma_counter <= 0)
 {
  ...
  if(DMAPending)
  {
   VDC_DEBUG("Deferred DMA starting(after SAT DMA)");
   DMAPending = false;
   DMARunning = 1;
   VDMA_CycleCounter = 0;
   DMAReadWrite = 0;
...
void VDC::IncRCR(void)
{
 ...
 if(VPhaseCounter <= 0)
 {
  ...
  switch(VPhase)
  {
   ...
   case VPHASE_VCR: NeedVBIRQTest = TRUE;
...
void VDC::DoVBIRQTest(void)
{
 ...
 {
  VDC_DEBUG("Deferred DMA starting(no SAT DMA, is this right?)");
  DMAPending = false;
  DMARunning = 1;
  VDMA_CycleCounter = 0;
  DMAReadWrite = 0;
...
void VDC::HDS_Start(void)
{
 ...
 {
  //printf("Sprite truncation on %d.  Wanted sprites: %d, cycles needed but not left: %d\n", RCRCount, active_sprites, sprite_cg_fetch_counter);
 ...
 // Moved to HDS PART3 ; See Final Zone II
 //DidRCRInc = FALSE;

 CR_cache = CR;

/*
 // Moved to HDW start to fix SF2 CE
 if(NeedVBIRQTest)
 {
  DoVBIRQTest();
  NeedVBIRQTest = FALSE;
...
int32 VDC::Run(int32 clocks, uint16 *pixels, bool skip)
{
 ...
 while(clocks > 0)
 {
  ...
  else if(DMARunning)	// && (VPhase != VPHASE_VDW || burst_mode)) // || ((HPhase == HPHASE_HDE || HPhase == HPHASE_HSW) /*&& !(CR_cache & 0x40)*/)     ))
  {
   if(VPhase != VPHASE_VDW || burst_mode || HPhase == HPHASE_HDW)
    RunDMA(chunk_clocks);
  }
  if(pixel_copy_count > 0) //(HPhase == HPHASE_HDW && VPhase == VPHASE_VDW && !burst_mode) || HPhase == HPHASE_MOO)
  {
   if(!skip)
   {
  ...
  {
   // Don't restart the horizontal sequence if we're already in external HSync
   if(HPhase == HPHASE_HSW && in_exhsync)
   {
...
uint8 VDC::Read(uint32 A, int32 &next_event, bool peek)
{
 ...
 switch(A)
 {
  case 0x0: ret = status | ((pending_read || pending_write) ? 0x40 : 0x00);	// | (VDC_IS_BSY ? 0x40 : 0x00);
...
uint16 VDC::Read16(bool A, bool peek)
{
 ...
 if(!A)
 {
  ret = status | ((pending_read || pending_write) ? 0x40 : 0x00);	// | (VDC_IS_BSY ? 0x40 : 0x00);
...
{
 if(sat_dma_counter <= 0 && !DMARunning/* && sprite_cg_fetch_counter <= 0*/)
 {
  if(pending_write)
  {
  ...
  case 0x3:
	   ...
	   //	VDC_DEBUG("%02x %d, %02x", select & 0x1F, msb, V);
           ...
           switch(select & 0x1F)
           {
			...
			// Not sure under which conditions deferred DMA is used(and if it even exists as such on a real system, much testing is needed).
			// Test games: Langrisser, Takeda Shingen(startup DMAs), Legendary Axe II(intro to stage 1, and stage 5 in particular).
			if((DCR & 0x10) && !burst_mode && VPhase == VPHASE_VDW)
			{
			 VDC_DEBUG("Deferred DMA");
			 DMAPending = 1;
			}
			else
			{
			 VDC_DEBUG("Instastart DMA");
			 DMAPending = 0;
                         DMARunning = 1;
			 VDMA_CycleCounter = 0;
       	                 DMAReadWrite = 0;
...
int VDC::StateAction(StateMem *sm, int load, int data_only, const char *sname)
{
 ...
 SFORMAT StateRegs[] = 
 {
	...
	SFVARN(VDS_cache, "VDS_cache"),
        ...
        SFVARN(VBlankFL_cache, "VBlankFL_cache"),
