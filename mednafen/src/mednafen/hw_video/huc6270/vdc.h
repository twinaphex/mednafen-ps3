...
#define VDC_IS_BSY	(pending_read || pending_write || sat_dma_counter > 0)
...
class VDC
{
	...
	INLINE int32 CalcNextEvent(void)
	{
	 ...
	 if(DMARunning && sat_dma_counter <= 0 && (VPhase != VPHASE_VDW || burst_mode))
	 {
	  ...
	  // A little better granularity.  (Though it's not really needed that much)
	  //if(next_vram_dma_event > 256)
	  // next_vram_dma_event = 256;
	...
	uint32 VDW_cache;
	uint32 VCR_cache;
	uint32 VBlankFL_cache;
	uint16 MWR_cache;


	uint32 BG_YMoo;
	bool DidRCRInc, NeedVBIRQTest, DidVBIRQTest;
	int HPhase, VPhase;
