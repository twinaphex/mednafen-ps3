...
void CALLBACK auGPUmakeSnapshot(void) {
	//	FILE *bmpfile;
	//	char filename[256];
...
/* disabled */
void CALLBACK auGPUcursor(int iPlayer, int x, int y) {
	//	if (iPlayer < 0)
	//		return;
	//	if (iPlayer > 7)
	//		return;
...
uint32_t CALLBACK auGPUreadStatus(void) // READ STATUS
{
...
uint32_t CALLBACK auGPUreadData(void) {
	uint32_t l;
	auGPUreadDataMem(&l, 1);
...
void CALLBACK auGPUwriteData(uint32_t gdata) {
	PUTLE32(&gdata, gdata);
	auGPUwriteDataMem(&gdata, 1);
}
...
long CALLBACK auGPUconfigure(void) {
	return 0;
}
...
long CALLBACK auGPUdmaChain(uint32_t * baseAddrL, uint32_t addr) {
	...
	do {
		...
		if (count > 0)
			auGPUwriteDataMem(&baseAddrL[dmaMem >> 2], count);
...
void CALLBACK auGPUabout(void) // ABOUT
{
	return;
}
...
long CALLBACK auGPUfreeze(uint32_t ulGetFreezeData, GPUFreeze_t * pF) {
	//----------------------------------------------------//
	...
	auGPUwriteStatus(g_gpu.ulStatusControl[0]);
	auGPUwriteStatus(g_gpu.ulStatusControl[1]);
	auGPUwriteStatus(g_gpu.ulStatusControl[2]);
	auGPUwriteStatus(g_gpu.ulStatusControl[3]);
	auGPUwriteStatus(g_gpu.ulStatusControl[8]); // try to repair things
	auGPUwriteStatus(g_gpu.ulStatusControl[6]);
	auGPUwriteStatus(g_gpu.ulStatusControl[7]);
	auGPUwriteStatus(g_gpu.ulStatusControl[5]);
	auGPUwriteStatus(g_gpu.ulStatusControl[4]);
...
void auGPUgetScreenPic(unsigned char * pMem) {
	/*
...
void CALLBACK auGPUshowScreenPic(unsigned char * pMem) {
}

void CALLBACK auGPUsetfix(uint32_t dwFixBits) {
	g_prim.dwEmuFixes = dwFixBits;
}

void CALLBACK auGPUvBlank(int val) {
