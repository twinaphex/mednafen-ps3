...
bool Memory::loadROM(std::istringstream& stream, const bool forceDmg) {
//ROBO: Don't need this anymore
//	defaultSaveBasePath = stripExtension(romfile);

//	File rom(romfile);
	MemoryFile rom(stream);
...
void Memory::loadSavedata() {
	...
	if (battery) {
//ROBO: Make it for mednafen
//		std::ifstream file((sbp + ".sav").c_str(), std::ios::binary | std::ios::in);
		std::string savefilename = MDFN_MakeFName(MDFNMKF_SAV, 0, "sav");
		std::ifstream file(savefilename.c_str(), std::ios::binary | std::ios::in);
...
//		std::ifstream file((sbp + ".rtc").c_str(), std::ios::binary | std::ios::in);
		std::string savefilename = MDFN_MakeFName(MDFNMKF_SAV, 0, "rtc");
		std::ifstream file(savefilename.c_str(), std::ios::binary | std::ios::in);
...
//		std::ofstream file((sbp + ".sav").c_str(), std::ios::binary | std::ios::out);
		std::string savefilename = MDFN_MakeFName(MDFNMKF_SAV, 0, "sav");
		std::ofstream file(savefilename.c_str(), std::ios::binary | std::ios::out);

		file.write(reinterpret_cast<const char*>(rambankdata), rambanks * 0x2000ul);
	}

	if (rtcRom) {
//ROBO: Make if for mednafen
